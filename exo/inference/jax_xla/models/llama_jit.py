...
def apply_rotary_embed(query: jax.Array, key: jax.Array, sin: jax.Array, cos: jax.Array, expand_axis = 2):
    cos = jnp.expand_dims(cos, axis=expand_axis)
    sin = jnp.expand_dims(sin, axis=expand_axis)
...
def rms_norm(hidden_state: jax.Array, weight: jax.Array, eps: jax.Array):
    ...
    variance = jnp.asarray(hidden_state, dtype=jnp.float32)
    variance = jnp.power(variance, 2)
    variance = variance.mean(-1, keepdims=True)
    # use `jax.numpy.sqrt` as `jax.lax.rsqrt` does not match `torch.rsqrt`
    hidden_state = hidden_state / jnp.sqrt(variance + eps)

    return weight * hidden_state.astype(input_dtype)

def _linear(
        input: jax.Array,
        weight: jax.Array,
        ):
    return lax.dot_general(input, weight.astype(jnp.float32), (((input.ndim - 1,), (0,)), ((), ())))
...
def mlp(hidden_states: jax.Array, up_proj: jax.Array, gate_proj: jax.Array, down_proj: jax.Array):
    ...
    return _linear(jax.nn.swish(gate) * up, down_proj)

@partial(jax.jit, static_argnames=['num_kv_heads', 'num_attention_heads', 'is_causal'])
def test_layer(
    ...
    hidden_state = hidden_state.astype(jax.dtypes.bfloat16)
    ...
    attn_out = jit_attention(q,k,v, bias=None, is_causal=is_causal, mask=None) 
